...
/release/        automation scripts for assembling the unified release bundle
...
After producing the platform installers and web bundle locally, run:
...
The script writes a fresh `Release/` directory containing every installer, the
offline web build, the autogenerated `Open_BioSpeak` launcher (plus a `.bat`
variant for Windows), and `release.json` metadata ready to be zipped.
