"""Terminal interface for the BioLang CLI."""
...
from biolang_core import BioLangEngine, CommandError, generate_file_map, run_self_tests
from biolang_core.portable import create_ready_bundle


def run_repl() -> None:
    engine = BioLangEngine()
    print("BioLang ready. Say commands like 'load dna file sample.fasta as sample'.")
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="BioLang natural language bioinformatics environment")
    parser.add_argument(
        "command",
        nargs="?",
...
def main(argv: list[str] | None = None) -> None:
    ...
    if args.command == "run":
        ...
        target = Path(args.path or "BioLangReady")
